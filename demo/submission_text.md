...
Built on **Perseus Vault**, a battle-tested persistent memory backend with SQLite + FTS5 full-text search. Real persistence, not a mock.
...
- **Python** — Executa plugin proxying Anna JSON-RPC calls to Perseus Vault
- **Perseus Vault** (Rust) — Open-source persistent memory backend
...
```bash
# Prerequisites: Perseus Vault installed (github.com/Perseus-Computing-LLC/perseus-vault)
npm i -g @anna-ai/cli
